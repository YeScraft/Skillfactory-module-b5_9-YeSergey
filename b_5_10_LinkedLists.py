class Node(object): # создаёт узел
    ...
    def reverse_node(self, other):
        new_other = Node(self.value)
        new_self = Node(other.value)
        self.value = new_self.value
        other.value = new_other.value
...
class LinkedList(): # создает связный список
    ...
    def sort_linked_list(self): # сортирует связный список от малого значения к большому
        first = self.head
        i = 1
        while i < self.length:
            second = first.next
            k = 1
            while k <= (self.length-i):
                if second.value < first.value:
                    first.reverse_node(second)
                    second = second.next
                else:
                    second = second.next
                k+=1
...
#linkedlist.head.reverse_node(linkedlist.head.next)
linkedlist.sort_linked_list()
linkedlist.print_list()
